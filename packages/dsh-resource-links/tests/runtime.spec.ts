...
describe('metadata-backed routing', () => {
  ...
  it('requires Host metadata for inline root and current directory, and never queries prose or authored paths', async () => {
    vi.useFakeTimers()
    const { runtime, gateway } = fixture({ resolveMany: vi.fn(async (_id, paths) => paths.map(inputPath => ({
      inputPath, ok: true as const, value: { path: inputPath, name: inputPath, kind: 'directory' as const },
    }))) })
    for (const mode of ['text', 'target'] as const) {
      expect(await runtime.resolve(id, '/', mode)).toEqual([])
      expect(await runtime.resolve(id, '.', mode)).toEqual([])
    }
    expect(gateway.resolveMany).not.toHaveBeenCalled()
    const found = Promise.all(['/', '.'].map(path => runtime.resolve(id, path, 'inline-code')))
    await vi.runAllTimersAsync()
    expect((await found).flat().map(link => link.target)).toEqual(['/', '.'])
    expect(gateway.resolveMany).toHaveBeenCalledWith(id, ['/', '.'], expect.any(AbortSignal))
    const unsupported = fixture({ resolveMany: async (_id, paths) => paths.map(inputPath => ({
      inputPath, ok: false as const, error: { code: inputPath === '/' ? 'EINVAL' : 'ENOENT', message: 'Host cannot resolve this path' },
    })) })
    const missing = Promise.all(['/', '.'].map(path => unsupported.runtime.resolve(id, path, 'inline-code')))
    await vi.runAllTimersAsync()
    expect((await missing).flat()).toEqual([])
  })
...
describe('bounded discovery lifetime', () => {
  it('delegates known sessions without projected cwd to the manager and separates that cache identity', async () => {
    ...
    const first = runtime.resolve(id, '/root/a@b.txt', 'inline-code')
    await vi.runAllTimersAsync()
    expect(await first).toHaveLength(1)
    cwd = '/root'
    const second = runtime.resolve(id, '/root/a@b.txt', 'inline-code')
    await vi.runAllTimersAsync()
    expect(await second).toHaveLength(1)
    expect(gateway.resolveMany).toHaveBeenCalledTimes(2)
    const directory = runtime.resolve(id, 'src', 'inline-code')
    await vi.runAllTimersAsync()
  ...
  it('deduplicates across text nodes and sends bounded batches', async () => {
    vi.useFakeTimers()
    const { runtime, gateway } = fixture()
    const a = Promise.all(['a.md', 'b.md', 'c.md'].map(path => runtime.resolve(id, path, 'inline-code')))
    const b = Promise.all(['a.md', 'd.md', 'e.md'].map(path => runtime.resolve(id, path, 'inline-code')))
    await vi.runAllTimersAsync()
    expect((await a).flat()).toHaveLength(3)
    expect((await b).flat()).toHaveLength(2)
  ...
  it('expires successful cache entries and isolates session and cwd identities', async () => {
    ...
    const resolve = async (session = id) => {
      const result = f.runtime.resolve(session, 'a.md', 'inline-code')
      await vi.runAllTimersAsync()
      return await result
    }
  ...
  it('evicts at the configured cache bound and revalidates cached links on click', async () => {
    ...
    for (const path of ['a.md', 'b.md', 'a.md']) {
      const result = runtime.resolve(id, path, 'inline-code')
  ...
  it('does not cache missing paths or transport failures', async () => {
    ...
    for (const expected of [0, 0, 1]) {
      const result = runtime.resolve(id, 'a.md', 'inline-code')
      await vi.runAllTimersAsync()
  ...
  it('discards in-flight old-cwd results', async () => {
    ...
    const result = f.runtime.resolve(id, 'a.md', 'inline-code')
  ...
  it('disposal aborts in-flight metadata and settles pending text without late links', async () => {
    ...
    const result = Promise.all(['a.md', 'b.md', 'c.md'].map(path => runtime.resolve(id, path, 'inline-code')))
    await vi.advanceTimersByTimeAsync(10)
    await runtime.dispose()
    expect(aborted).toBe(true)
    expect((await result).flat()).toEqual([])
    expect(await runtime.resolve(id, 'a.md', 'inline-code')).toEqual([])
