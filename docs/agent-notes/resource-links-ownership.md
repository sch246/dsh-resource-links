...
Automatic path discovery requires the Host's parsed `inline-code` context. Text scanning only retains explicit session references; authored destinations use a separate `target` context. Nearby, escaped or unmatched backticks cannot establish code-span identity. Root and current-directory tokens still require successful Host metadata, because parser acceptance alone cannot prove a target exists on the Host. Explicit resource-open actions retain their path handling independently of discovery.
