...
## Stable behavior and limits

- Displayed message text may link metadata-confirmed existing files and directories relative to the owning Session workspace. Discovery accepts absolute POSIX paths, explicit relative paths, slash-containing relative paths and filename-extension tokens. Quotes and explicit Markdown destinations can preserve spaces; explicit destinations also accept extensionless basenames. Ordinary bare words, ambiguous unquoted paths with spaces, nonexistent and inaccessible paths remain inert. Discovery never scans directories recursively or reads file contents.
...
- Existing absolute, relative, inline-code and spaced explicit paths become links; directories mount Files, text files mount the selected editor, and an image uses the matching handler. Nonexistent/inaccessible and ambiguous candidates remain plain text. Selecting a session reference changes the addressed session without filesystem resolution.
...
The linked execution and managed-activation logs retain candidate tests, cold-browser observations and the recovered private removal probe. They do not establish user visual acceptance, portability to an arbitrary Host or transfer of historical receipts. This STATE update changes documentation only; no build, install, removal, filesystem mutation or service activation is performed.
