...
Large text files require an explicit confirmation opportunity rather than a permanent refusal. Shared file access checks the existing file size before reading content and reports the actual size and configured threshold when confirmation is needed. Confirmation governs loading existing disk content, including save revision checks. Local edits may grow beyond the threshold and save successfully without another prompt; byte operations retain their configured bound. Consumers carry explicit confirmation on each related text request and preserve cancellation and guarded-save behavior. The [package reference](../../packages/dsh-user-files/README.md) owns the request flag and typed error fields.
