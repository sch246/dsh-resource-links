...
Confirmation applies to each text request, including guarded saves and refreshes. It does not authorize byte operations or weaken UTF-8 validation, revision comparison, EOL restoration or atomic publication. The same provider queue and read implementation serve confirmed and unconfirmed requests; there is no second large-file storage path.
